...
export class AIProcessingClient implements AIProcessingPort {
  ...
  async search(query: string, filters?: SearchFilters): Promise<SearchResult> {
    ...
    if (!response.ok) {
      throw new Error(
        `AI Processing search failed: ${response.status} ${response.statusText}`
      );
    }
    ...
    return {
      courses: data.courses.map((c) => CourseFactory.create(c as Parameters<typeof CourseFactory.create>[0])),
  ...
  async chat(message: string, sessionId?: string): Promise<ChatResult> {
    ...
    if (!response.ok) {
      throw new Error(
        `AI Processing chat failed: ${response.status} ${response.statusText}`
      );
    }
