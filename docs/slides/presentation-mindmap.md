...
```
課題背景 + 問題 → 誰のために → Lumineer とは → システム構成 → 見せる → こだわり → 感謝
 (Background)     (Persona)   (Introduction)  (Arch/RAG)   (Demo)   (Excellence) (Closing)
```

---

## 1. Background & Problem：課題の背景と問題

> ※ VP レベルの審査員は課題詳細を事前に見ていない前提で、背景から丁寧に説明する

### 与えられた課題

> "Coursera の 6,645 件のコースデータを使って、AI エージェントによるインテリジェントなコース発見システムを作れ"

- 課題の要件書には Req1（Basic）と Req2（Advanced）の 2 段階が設定されていた
- 各判断は ADR（13 本）として記録済み
...
### この問題を解決するために作ったのが — **Lumineer（ルミニア）**
...
## 3. Lumineer とは：システム全体像

> この問題を解くために構築したシステムを紹介する。「どんな構成か」を一枚で伝えるスライド。

- **Lumineer の全体構成**
  - `Frontend → Gateway → Backend → AI Processing → Qdrant / OpenAI`
  - 月額インフラコスト：約 $6（Cloud Run + Qdrant Cloud 無料枠を最大活用）
...
**語り方：**
- 「問題を解くために、4 つのレイヤーで構成されたシステムを作りました」
- 「各レイヤーの役割と技術選定の理由を、次から順番に説明します」
...
**発表時間：15分（デモ含む）+ Q&A 5分**
> ※ 課題PDF記載（8分+2分）とは異なる。Prodapt 側の設定に従う。

**15分構成：**

| # | セクション | 時間 | 累計 |
|---|-----------|------|------|
| 1 | Background & Problem（課題背景 + 問題 + 要件対応表） | 1:00 | 0:00-1:00 |
| 2 | Persona | 0:40 | 1:00-1:40 |
| 3 | Lumineer とは（システム全体像） | 0:30 | 1:40-2:10 |
| 4 | Tech Stack | 0:45 | 2:10-2:55 |
| 5 | Architecture | 1:15 | 2:55-4:10 |
| 6 | RAG Pipeline & Guardrails | 2:00 | 4:10-6:10 |
| 7 | Validation | 0:30 | 6:10-6:40 |
| 8 | Live Demo | 3:30 | 6:40-10:10 |
| 9 | Engineering Excellence | 2:30 | 10:10-12:40 |
| 10 | Closing | 0:30 | 12:40-13:10 |
| | **バッファ** | **1:50** | **13:10-15:00** |

**Q&A 想定時間：5分**
- 質問は2〜3問を想定
- 深い技術質問が来ても「ADR に記録してある」「デモで確認できる」で対応可能
