...
└─────────────────────┬───────────────────────────────┘
                      │ HTTP
┌─────────────────────▼───────────────────────────────┐
│  AI Processing Layer: Python + Litestar (or FastAPI) │
│  Cloud Run / OpenAI ベストプラクティス                  │
│                                                      │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐           │
│  │ Agents   │  │ RAG      │  │ MCP      │           │
│  │ (SDK)    │  │ Pipeline │  │ Server   │           │
...
| Explore | `/explore` | コースカタログ。検索バー + フィルタ + カードグリッド + AI 要約 |
| Chat | `/chat` | AI 対話。検索・スキル分析・推薦 |
| My Path | `/path` | 学習パス管理・可視化 |
| Course Detail | `/course/:id` | コース詳細 |
| Settings | `/settings` | パイプライン設定（リランカー、フォーマット等）の UI 変更 |
...
- ローカル Qdrant コンテナが起動（外部サービス依存なし、OpenAI API 以外）
- デフォルト値で全て動作
