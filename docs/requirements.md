...
**構成**: マイクロサービス + API Gateway + Docker Compose

```mermaid
graph TD
    FE["**Frontend**<br/>Bun + React (Vite) + Shadcn UI<br/>Firebase Hosting (CDN)"]
    GW["**API Gateway**<br/>Bun + Hono (TypeScript)<br/>Cloud Run / 外部公開唯一のエントリポイント<br/>CORS・ログ・レート制限・Proxy"]
    API["**Backend Service**<br/>Bun + Hono (TypeScript)<br/>Cloud Run / クリーンアーキテクチャ<br/>内部のみ公開"]
    AI["**AI Processing Service**<br/>Python + Litestar / Cloud Run<br/>内部のみ公開"]
    AG["Agents (SDK)"]
    RAG["RAG Pipeline"]
    PG["**PostgreSQL**<br/>ユーザー・チャット<br/>学習パス・設定<br/>Cloud SQL / Docker"]
    QD["**Qdrant Cloud**<br/>(VectorDB)"]
    OBS["**Observability**<br/>Langfuse + Grafana<br/>+ GCP Cloud Monitoring"]
...
- **API Gateway（薄いルーティング層）**: CORS・ログ・レート制限・proxy のみ。ビジネスロジックなし
- **Backend Service（クリーンアーキテクチャ）**: domain → infrastructure → interfaces の依存方向
...
| API Gateway | Bun + Hono (TypeScript) | 薄いルーティング層 / CORS・ログ・レート制限・Proxy |
| Backend Service | Bun + Hono (TypeScript) | クリーンアーキテクチャ / 内部のみ公開 |
...
sequenceDiagram
    ...
    Note over C,DB: 1. ユーザー登録
    C->>API: POST /api/auth/register<br/>{email, password, display_name}
    API->>DB: bcrypt ハッシュ化 → 保存
    API-->>C: JWT (Access Token + Refresh Token)

    Note over C,DB: 2. ログイン
    C->>API: POST /api/auth/login {email, password}
    API->>DB: bcrypt でパスワード照合
    API-->>C: JWT (Access Token + Refresh Token)

    Note over C,DB: 3. 認証付きリクエスト
    C->>API: GET /api/chat/sessions<br/>Authorization: Bearer <access_token>
    API->>API: Hono middleware で JWT 検証
    API-->>C: user_id をコンテキストに注入してレスポンス

    Note over C,DB: 4. トークンリフレッシュ
    C->>API: POST /api/auth/refresh {refresh_token}
    API-->>C: 新しい Access Token
```
...
```mermaid
graph TD
    subgraph ING["Ingestion（初回のみ）"]
        CSV["CSV"] --> LLM["LLM 前処理<br/>(GPT-4o-mini)"]
        LLM --> EMB["Embedding<br/>(text-embedding-3-large)"]
        EMB --> QD1["Qdrant"]
    end

    subgraph SEARCH["検索（リクエストごと）"]
        Q["Query"] --> S1["Step 1: メタデータフィルタ<br/>(Level, Organization, rating 等)"]
        S1 --> S2["Step 2: Hybrid Search<br/>(Dense + Sparse 並列)"]
        S2 --> S3["Step 3: RRF スコア統合<br/>→ top_k=20"]
        S3 --> S4["Step 4: Reranking<br/>(Strategy パターンで切替)"]
        S4 --> S5["Step 5: Result Selection<br/>(Top-k + Threshold)"]
        S5 -->|"件数不足時"| COR["Corrective RAG<br/>(クエリ再構成→再検索)"]
        COR --> S2
        S5 --> S6["Step 6: Context Assembly<br/>(JSON or TOON フォーマット)"]
        S6 --> AGT["Agent に返却 → LLM が回答生成"]
    end
```
...
```mermaid
graph TD
    U["ユーザー入力"] --> T["Triage Agent<br/>入力を分類・ルーティング"]
    T -->|"handoff()"| S["Search Agent<br/>コース検索"]
    T -->|"handoff()"| SG["Skill Gap Agent<br/>スキル分析"]
    T -->|"handoff()"| P["Path Agent<br/>学習パス"]
```
...
```mermaid
graph TD
    OUT["LLM 出力"] --> S1["Stage 1: DB 照合<br/>検索結果 Set 比較<br/>コスト 0 / レイテンシ ~0ms"]
    S1 -->|"コース名が retrieved_courses に不在"| NG1["捏造として検出"]
    S1 -->|"データ変更が疑われる場合"| QDF["Qdrant フィルタ検索<br/>(~5-10ms)"]
    S1 --> S2["Stage 2: LLM Verifier<br/>@output_guardrail / 並列実行<br/>コンテキスト忠実性チェック"]
    S2 -->|"OK"| OK["ユーザーへ返却"]
    S2 -->|"NG"| NG2["ブロック / 再生成"]
```
...
```mermaid
graph TD
    PR["Push / PR"] --> S1["Stage 1: Lint + Format + Unit Test（全 Push）<br/>Frontend: prettier + eslint + tsc + vitest<br/>Backend: prettier + eslint + tsc + vitest<br/>AI: ruff format + ruff check + mypy + pytest"]
    S1 -->|"LLM 関連変更あり<br/>(paths フィルタ)"| S2["Stage 2: LLM Eval<br/>DeepEval + Golden Dataset"]
    S1 -->|"LLM 関連変更なし"| SKIP["LLM Eval スキップ"]
    S2 -->|"Tier 1 閾値未満"| BLOCK["マージブロック"]
    S2 -->|"Pass"| S3["Stage 3: Deploy<br/>（main マージ後）<br/>Docker build → Cloud Run デプロイ"]
```
...
```mermaid
graph LR
    M["main<br/>本番（Cloud Run 自動デプロイ）"]
    D["develop<br/>統合（feature 統合先）"]
    F["LM0001-feature/rag-hybrid_search"]
    M --- D
    D --- F
```
...
graph LR
    subgraph NORMAL["通常フロー"]
        D1["develop"] --> FB["feature branch"]
        FB --> PR1["PR"] --> CI["CI"] --> D2["develop"] --> MA1["main"]
    end
    subgraph HOTFIX["hotfix フロー"]
        MA2["main"] --> HF["hotfix branch"]
        HF --> PR2["PR"]
        PR2 --> MA3["main"]
        PR2 --> D3["develop マージ"]
    end
...
```mermaid
graph LR
    UI["Web UI<br/>(Explore/Chat)"] --> TA["Triage Agent"]
    MC["MCP Client"] --> TA
    TA --> RAG["同じ RAG パイプライン"] --> ANS["回答"]
```
