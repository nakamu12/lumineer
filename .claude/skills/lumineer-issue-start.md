...
※ Draft PR は作成しない。実装完了後にユーザーが手動で PR を作成する。
...
### 4-3. .env.local のシンボリックリンクを作成

`.env.local` は gitignore 対象のため worktree には存在しない。メインリポジトリの `.env.local` へのシンボリックリンクを張る:

```bash
# project_root から worktree への相対パスでシンボリックリンクを作成
ln -s ../../lumineer/.env.local {worktree_path}/.env.local
```

メインリポジトリに `.env.local` が存在する場合のみ実行する。存在しない場合はスキップしてユーザーに通知する。

### 4-4. scope に応じた依存関係インストール
...
実装完了後は gh pr create --base develop で PR を作成してください。
```
